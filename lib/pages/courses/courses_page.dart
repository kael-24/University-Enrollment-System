      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('${cp.courses.length} available courses', style: AppTextStyles.caption),
            const SizedBox(height: 16),
            CustomSearchBar(controller: _searchCtrl, hintText: 'Search by code, title, or instructor...', onChanged: (v) => setState(() => _query = v)),
                        ...
                        return GlassCard(
                          ...
                          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Row(children: [
                              ...
                              const Spacer(),
                              if (isFull) Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: AppColors.error.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(12), border: Border.all(color: AppColors.error.withValues(alpha: 0.3))), child: const Text('FULL', style: TextStyle(color: AppColors.error, fontSize: 11, fontWeight: FontWeight.bold))),
                              if (!isFull) Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4), decoration: BoxDecoration(color: AppColors.success.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(12)), child: Text('${c.capacity - enrolled} slots', style: const TextStyle(color: AppColors.success, fontSize: 11, fontWeight: FontWeight.w600))),
                            ]),
                            const SizedBox(height: 4),
                            ...
                            Row(children: [
                              ...
                              Text(c.instructor, style: AppTextStyles.caption),
                              const SizedBox(width: 16),
                              const Icon(Icons.schedule, size: 14, color: AppColors.textSecondary),
                              const SizedBox(width: 4),
                              Text(c.schedule, style: AppTextStyles.caption),
                              const SizedBox(width: 16),
                              Text('${c.units} units', style: AppTextStyles.caption.copyWith(fontWeight: FontWeight.w600)),
                            ]),
