...
class _StudentEnrollmentsPageState extends State<StudentEnrollmentsPage>
      ...
      itemBuilder: (ctx, i) {
        ...
        return Dismissible(
          ...
          confirmDismiss: (_) => showConfirmDialog(
            context,
            title: 'Request drop for ${course.courseCode}?',
            message:
                'Your drop request for "${course.title}" will be sent to the professor for approval.',
          ),
          onDismissed: (_) {
            ep.requestDropEnrollment(e.id);
            showAppSnackbar(
              context,
              message: 'Drop request submitted for ${course.courseCode}',
  ...
  Widget _buildPendingTab(List pending, CourseProvider cp) {
    if (pending.isEmpty)
      return const EmptyState(
        ...
        subtitle: 'Your enrollment requests will appear here',
      );
    return ListView.builder(
  ...
  Widget _buildHistoryTab(List history, CourseProvider cp) {
    if (history.isEmpty)
      return const EmptyState(
        icon: Icons.history,
        title: 'No history',
        subtitle: 'Completed and dropped courses appear here',
      );
    return ListView.builder(
            ...
            children: [
              Container(
                ...
                child: Icon(
                  ...
                  color: e.status == EnrollmentStatus.completed
                      ? AppColors.secondary
                      : AppColors.error,
                  size: 20,
                ),
