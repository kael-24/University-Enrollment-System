...
class _StudentCoursesPageState extends State<StudentCoursesPage> {
              ...
              Expanded(
                child: list.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (ctx, i) =>
                            _buildCourseCard(list[i], studentId, ep, cp),
                      ),
              ),
            ],
          ),
  ...
  Widget _buildCourseCard(
    ...
    if (course.prerequisiteCourseId != null) {
      hasPrereq = ep.hasStudentCompletedCourse(
        studentId,
    ...
    final canEnroll =
        !isFull && !isEnrolled && !isPending && hasPrereq && !hasCompleted;
        ...
        children: [
          ...
          if (course.prerequisiteCourseId != null) ...[
            ...
            Row(
              children: [
                Icon(
                  hasPrereq ? Icons.check_circle : Icons.warning_amber_rounded,
                  size: 14,
                  color: hasPrereq ? AppColors.success : AppColors.warning,
                ),
                const SizedBox(width: 6),
                Expanded(
                  child: Text(
                    hasPrereq
                        ? 'Prerequisite ${cp.getCourseById(course.prerequisiteCourseId!)?.courseCode ?? ''} ✓'
                        : prereqWarning ?? 'Prerequisite not met',
  ...
  void _requestEnrollment(
    ...
    final confirmed = await showConfirmDialog(
      context,
      title: 'Enroll in ${course.courseCode}?',
      message:
          'Your request for "${course.title}" will be sent for professor approval.',
    );
    if (confirmed != true || !mounted) return;
    final result = ep.requestEnrollment(
      studentId: studentId,
      courseId: course.id,
