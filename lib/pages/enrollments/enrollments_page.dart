...
class _EnrollmentsPageState extends State<EnrollmentsPage> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('${ep.enrollments.length} total records', style: AppTextStyles.caption),
            ...
            Container(
              ...
              child: TabBar(
                ...
                tabs: const [Tab(text: 'All'), Tab(text: 'Active'), Tab(text: 'Dropped'), Tab(text: 'Done')],
              ),
            ),
            const SizedBox(height: 16),
            Expanded(
      ...
      floatingActionButton: FloatingActionButton(onPressed: () => showEnrollmentDialog(context), backgroundColor: AppColors.primary, child: const Icon(Icons.add_rounded)),
    );
  }

  Widget _buildList(List enrollments, StudentProvider sp, CourseProvider cp, EnrollmentProvider ep) {
    ...
    if (_query.isNotEmpty) {
      ...
      filtered = enrollments.where((e) {
        ...
        return (student?.fullName.toLowerCase().contains(q) ?? false) || (course?.courseCode.toLowerCase().contains(q) ?? false) || (course?.title.toLowerCase().contains(q) ?? false);
      }).toList();
    }
