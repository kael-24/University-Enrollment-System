...
class _ProfessorStudentsPageState extends State<ProfessorStudentsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final managedCourseIds = auth.isAdmin
        ? cp.courses.map((c) => c.id).toSet()
        : cp
    ...
    if (!auth.isAdmin) {
      list = list
          .where(
            (s) => ep
                .getEnrollmentsForStudent(s.id)
                .any((e) => managedCourseIds.contains(e.courseId)),
          )
          .toList();
    }
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                auth.isAdmin ? 'Students' : 'My Course Students',
                style: AppTextStyles.heading,
              ),
              const SizedBox(height: 4),
              Text(
                '${list.length} visible students',
                ...
                child: list.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (ctx, i) {
                          ...
                          final enrollments = ep
                              .getEnrollmentsForStudent(s.id)
                              .where(
                                (e) => managedCourseIds.contains(e.courseId),
                              )
                              .toList();
                          ...
                          return GlassCard(
                            onTap: () => setState(
                              () =>
                                  _expandedStudentId = isExpanded ? null : s.id,
                            ),
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                                  ...
                                                  final confirmed =
                                                      await showConfirmDialog(
                                                        ...
                                                        title:
                                                            'Mark as Completed?',
                                                        message:
                                                            'Mark ${s.fullName}\'s enrollment in ${course.courseCode} as completed?',
                                                      );
                                                  if (confirmed == true) {
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () {
                      if (fn.text.isNotEmpty && ln.text.isNotEmpty) {
                        final id = context.read<StudentProvider>().addStudent(
