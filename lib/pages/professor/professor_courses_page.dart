...
class _ProfessorCoursesPageState extends State<ProfessorCoursesPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final managedCourses = auth.isAdmin
        ? cp.courses
        : cp.getCoursesForProfessor(auth.currentUserId ?? '');
    var list = _query.isNotEmpty
        ? managedCourses.where((c) {
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                auth.isAdmin ? 'Manage All Courses' : 'My Managed Courses',
                          ...
                          return GlassCard(
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 4),
                                Text(
                                  '${c.units} units • ${c.category.shortLabel}',
  ...
  void _showAddCourse(BuildContext context, CourseProvider cp) {
    ...
    final professor = context.read<ProfessorProvider>().getProfessorById(
      auth.currentUserId ?? '',
    );
    ...
    final instrCtrl = TextEditingController(
      text: auth.isProfessor && professor != null ? professor.fullName : '',
    );
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    labelText: 'Schedule',
                    hintText: 'e.g., MWF 9:00-10:00 AM',
                      ...
                      onPressed: () {
                        if (codeCtrl.text.isNotEmpty &&
                          ...
                          cp.addCourse(
                            ...
                            instructor: instrCtrl.text.trim(),
                            professorId: auth.isProfessor
                                ? auth.currentUserId
                                : null,
                            category: cat,
                          );
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        cp.updateCourse(
                          ...
                          description: descCtrl.text.trim(),
                          capacity:
                              int.tryParse(capCtrl.text) ?? course.capacity,
                          units: int.tryParse(unitsCtrl.text) ?? course.units,
                          schedule: schedCtrl.text.trim(),
                          instructor: instrCtrl.text.trim(),
                          category: cat,
                        );
  ...
  void _deleteCourse(
    ...
    final confirmed = await showConfirmDialog(
      ...
      message:
          'This will permanently remove "${course.title}". This action cannot be undone.',
    );
